...
function seed() {
  ...
  // Åpne ordre (venter på planlegging)
  ...
  insertOrder.run(4, 'inspection', 'open', 2.5, 61.450, 5.833, null, null, null, 'Rutineinspeksjon av elektrisk anlegg');
  insertOrder.run(7, 'årskontroll', 'open', 4, 69.649, 18.956, null, null, null, 'Sensitiv årskontroll - krever clearance');
  insertOrder.run(9, 'årskontroll', 'open', 3.5, 70.663, 23.678, null, null, null, 'Årlig kontroll i Finnmark');

  // Planlagte ordre (for å skape bookinger)
  ...
  const orderId2 = insertOrder.run(5, 'inspection', 'planlagt', 3, 61.500, 5.900, 4, '2026-08-10T10:00:00Z', '2026-08-10T13:00:00Z', 'Inspeksjon av solcelleanlegg').lastInsertRowid;
  const orderId3 = insertOrder.run(6, 'årskontroll', 'planlagt', 3, 69.680, 18.900, 5, '2026-08-11T09:00:00Z', '2026-08-11T12:00:00Z', 'Årlig kontroll - sensitiv kunde').lastInsertRowid;
  ...
  insertBooking.run(1, 1, '2026-08-15T08:00:00Z', '2026-08-15T11:00:00Z'); // Fremtidig booking
  
  // Kari Teknisk (ID 2)
  insertBooking.run(2, orderId1, '2026-08-10T08:00:00Z', '2026-08-10T10:00:00Z');
  insertBooking.run(2, 2, '2026-08-14T10:00:00Z', '2026-08-14T12:00:00Z'); // Fremtidig booking
  
  // Per Fiksit (ID 3)
  insertBooking.run(3, orderId5, '2026-08-13T08:00:00Z', '2026-08-13T09:30:00Z');
  insertBooking.run(3, 3, '2026-08-16T09:00:00Z', '2026-08-16T11:00:00Z'); // Fremtidig booking
  
  // Lars Fjord (ID 4)
  insertBooking.run(4, orderId2, '2026-08-10T10:00:00Z', '2026-08-10T13:00:00Z');
  insertBooking.run(4, 4, '2026-08-17T10:00:00Z', '2026-08-17T12:00:00Z'); // Fremtidig booking
  
  // Morten Polar (ID 5)
  insertBooking.run(5, orderId3, '2026-08-11T09:00:00Z', '2026-08-11T12:00:00Z');
  insertBooking.run(5, 7, '2026-08-18T08:00:00Z', '2026-08-18T11:00:00Z'); // Fremtidig booking
  
  // Anne Nordlys (ID 6)
  insertBooking.run(6, 8, '2026-08-12T09:00:00Z', '2026-08-12T11:00:00Z'); // Fremtidig booking for Finnmark
  insertBooking.run(6, 9, '2026-08-19T10:00:00Z', '2026-08-19T13:00:00Z'); // Fremtidig booking
  ...
  // Sjekkliste for ordre 7 (Hemmeligholdt Kunde - sensitiv)
  insertChecklist.run(7, 'Sikkerhetssjekk av inngangssystem', 0);
  insertChecklist.run(7, 'Verifiser alarmfunksjonalitet', 0);
}
      ...
      for (const g of gaps) {
        ...
        const slotInfo = calculateSlotScore({
          ...
          gap: {
            ...
            prevLat: prevBooking ? prevBooking.lat : tech.base_lat,
            prevLng: prevBooking ? prevBooking.lng : tech.base_lng,
            nextLat: nextBooking ? nextBooking.lat : tech.base_lat,
            nextLng: nextBooking ? nextBooking.lng : tech.base_lng
          },
          newJob: {
            lat: order.lat,
            lng: order.lng,
...
// Hent kunder med full info
function getFullCustomerById(id) {
  return db.prepare('SELECT * FROM customers WHERE id = ?').get(id);
}

// Oppdater ordre med full info
...
module.exports = {
  ...
  getCustomerById: getFullCustomerById,
